...
def scrape_equipo_completo(url, equipo_nombre):
    ...
    try:
        ...
        # 1. Identificar quiénes son lanzadores en este reporte
        pitchers_names = []
        ...
        # 2. Extraer datos de Bateo Situacional
        if "BATTING ANALYSIS" in texto:
            seccion = texto.split("BATTING ANALYSIS")[1]
            lineas = seccion.split('\n')
            # Pattern: Nombre, vsRight, w/Runners, BasesEmpty, FlyOut, GndOut, Ratio
            pattern = r'^\s*\d*\s*([A-Za-z\s\.,]+)\.+\s+[\d-]+\s+[\d-]+\s+[\w.-]+\s+[\d-]+\s+[\d-]+\s+([\w.-]+)\s+[\d-]+\s+[\d-]+\s+([\w.-]+)\s+[\d-]+\s+[\d-]+\s+([\w.-]+)\s+[\d-]+\s+[\d-]+\s+[\w.-]+\s+(\d+)\s+(\d+)\s+([\d.]+)'
            ...
            for l in lineas:
                ...
                if match:
                    ...
                    posicion = "PITCHER" if nombre in pitchers_names else "INFIELD"
